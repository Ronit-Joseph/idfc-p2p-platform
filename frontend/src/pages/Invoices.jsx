...
import { Receipt, Shield, AlertTriangle, Clock, CheckCircle, XCircle, Eye } from 'lucide-react'
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Replaces Oracle EBS Invoice Approval UI · Click any invoice for full detail</p>
        </div>
            ...
            {filtered.map(inv => (
              <tr key={inv.id}
                ...
                <td className="px-3 py-3">
                  <div className="font-mono text-xs text-blue-700 font-medium">{inv.invoice_number}</div>
                  <div className="text-xs text-gray-400 mt-0.5">{inv.invoice_date}</div>
                </td>
                <td className="px-3 py-3 max-w-[140px]">
                  <div className="text-xs font-medium text-gray-800 truncate">{inv.supplier_name}</div>
                  {inv.is_msme_supplier && <span className="badge badge-orange text-[10px]">MSME {inv.msme_category}</span>}
                </td>
                <td className="px-3 py-3">
                  <div className="font-medium text-sm">{fmtInr(inv.net_payable)}</div>
                  <div className="text-xs text-gray-400">+GST {fmtInr(inv.gst_amount)}</div>
                </td>
                <td className="px-3 py-3">
                  {inv.gstin_validated_from_cache ? (
                    <div>
                      <span className="badge badge-green text-[10px]">✓ Cache</span>
                      <div className="text-xs text-gray-400 mt-0.5">{inv.gstin_cache_age_hours?.toFixed(1)}h ago</div>
                    </div>
                  ) : inv.gstin_cache_status ? (
                    <span className="badge badge-blue text-[10px]">Checking</span>
                  ) : <span className="text-gray-300 text-xs">—</span>}
                  {inv.gstr2b_itc_eligible === false && <div className="text-red-500 text-[10px] mt-0.5">ITC risk</div>}
                </td>
                <td className="px-3 py-3">
                  {inv.match_status !== 'PENDING' && inv.match_status ? (
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${MATCH_COLOR[inv.match_status] || 'text-gray-400 bg-gray-50'}`}>
                ...
                <td className="px-3 py-3">
                  {inv.coding_agent_gl ? (
                    <div>
                      ...
                      <div className="text-[10px] text-gray-400">{inv.coding_agent_confidence?.toFixed(0)}% conf.</div>
                    </div>
                  ) : <span className="text-gray-300 text-xs">—</span>}
                </td>
                <td className="px-3 py-3">
                  ...
                  {inv.fraud_flag && <div className="text-[10px] text-red-600 font-bold mt-0.5">🚨 FRAUD</div>}
                </td>
                <td className="px-3 py-3">
                ...
                <td className="px-3 py-3">
                  <Eye className="w-4 h-4 text-gray-300 hover:text-blue-500" />
                </td>
              </tr>
            ))}
          </tbody>
