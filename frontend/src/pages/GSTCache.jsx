...
export default function GSTCache() {
  ...
  if (!data) return <div className="text-gray-400 py-8 text-center">Loading GST Cache…</div>
          ...
          <tbody className="divide-y divide-gray-50">
            {filtered.map(g => (
              <tr key={g.gstin} className={`text-sm ${!g.gstr2b_available || g.gstr1_compliance === 'DELAYED' ? 'bg-yellow-50' : 'hover:bg-gray-50'}`}>
                ...
                <td className="px-3 py-2.5">
                  {g.gstr2b_available
                    ? <span className="badge badge-green text-[10px]">✓ {g.gstr2b_period}</span>
                    : <div>
                        <span className="badge badge-red text-[10px]">✗ Missing</span>
