...
export default function MSMECompliance() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-gray-900">MSME Compliance Dashboard</h1>
        <p className="text-sm text-gray-500 mt-0.5">{summary.section_43bh} · Max {summary.max_payment_days} days · Penalty: {summary.penalty_multiplier}× RBI rate ({summary.rbi_rate}%)</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="card text-center">
          <div className="text-3xl font-bold text-gray-800">{summary.total_msme_invoices}</div>
          <div className="text-xs text-gray-500 mt-1">Total MSME Invoices</div>
        </div>
          ...
          <tbody className="divide-y divide-gray-50">
            {invoices.map(inv => (
              <tr key={inv.invoice_id}
                ...
                <td className="px-4 py-3 text-xs font-medium text-gray-800">{inv.supplier_name}</td>
                <td className="px-4 py-3">
                  <span className="badge badge-orange text-[10px]">{inv.msme_category}</span>
                </td>
                <td className="px-4 py-3 text-xs text-gray-500">{inv.invoice_date}</td>
                <td className="px-4 py-3 font-medium text-sm">{fmtInr(inv.invoice_amount)}</td>
                <td className="px-4 py-3">
                  <span className={`badge text-[10px] ${inv.invoice_status === 'APPROVED' || inv.invoice_status === 'POSTED_TO_EBS' ? 'badge-green' : 'badge-yellow'}`}>
                    {inv.invoice_status?.replace(/_/g,' ')}
                  </span>
                </td>
                <td className="px-4 py-3 text-xs text-gray-500">{inv.msme_due_date}</td>
                <td className="px-4 py-3">
                  ...
                  <span className="text-xs text-gray-400 ml-1">{inv.days_remaining < 0 ? 'overdue' : 'remaining'}</span>
                </td>
                <td className="px-4 py-3">
