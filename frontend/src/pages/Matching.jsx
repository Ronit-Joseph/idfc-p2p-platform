...
export default function Matching() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-gray-400">Loading matching data…</div>

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Invoice Matching Engine</h1>
          <p className="text-sm text-gray-500 mt-0.5">2-Way (PO vs Invoice) & 3-Way (PO vs GRN vs Invoice) matching · 5% tolerance · Fraud detection</p>
        </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="bg-white rounded-xl border border-blue-100 p-5">
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Total Matches</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">{summary.total_matches}</p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-yellow-100 p-5">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Exception</p>
            </div>
          ...
          <div className="bg-white rounded-xl border border-orange-100 p-5">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Open Exceptions</p>
            </div>
      ...
      {tab === 'results' ? (
        <div className="card p-0 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {results.length === 0 ? (
                <tr><td colSpan={7} className="px-3 py-8 text-center text-gray-400 text-sm">No match results yet</td></tr>
              ) : results.map(r => (
                <tr key={r.id} className="table-row-hover">
                  <td className="px-3 py-3 font-mono text-xs text-blue-700 font-medium">{r.invoice_number}</td>
                  <td className="px-3 py-3 text-xs text-gray-700">{r.supplier_name || '—'}</td>
                  <td className="px-3 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${MATCH_TYPE_BADGE[r.match_type] || 'bg-gray-100'}`}>
                      {r.match_type}
                    </span>
                  </td>
                  <td className="px-3 py-3">
                  ...
                  <td className="px-3 py-3 text-xs text-gray-500">{r.note || r.exception_reason || '—'}</td>
        ...
        <div className="space-y-3">
          {exceptions.length === 0 ? (
            ...
            <div key={ex.id} className={`rounded-xl border p-4 ${EXCEPTION_SEVERITY[ex.severity] || 'bg-gray-50 border-gray-200'}`}>
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
