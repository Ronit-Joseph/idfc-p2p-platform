...
export default function TDSManagement() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-gray-400">Loading TDS data…</div>

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">TDS Management</h1>
          <p className="text-sm text-gray-500 mt-0.5">Tax Deducted at Source · Section 194C / 194J / 194H / 194I · Auto-calculation with 4% H&E Cess</p>
        </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl border border-purple-100 p-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">{summary.total_deductions}</p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-red-100 p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Total TDS Amount</p>
                <p className="text-2xl font-bold text-gray-900 mt-1">{fmtInr(summary.total_tds_amount)}</p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-blue-100 p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Deposited</p>
                <p className="text-2xl font-bold text-gray-900 mt-1">{summary.deposited || 0}</p>
              </div>
          ...
          <div className="bg-white rounded-xl border border-green-100 p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Form 16A Pending</p>
                <p className="text-2xl font-bold text-gray-900 mt-1">{summary.form16a_pending || 0}</p>
              </div>
      ...
      {tab === 'deductions' ? (
        <div className="card p-0 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {deductions.length === 0 ? (
                <tr><td colSpan={10} className="px-3 py-8 text-center text-gray-400 text-sm">No TDS deductions yet</td></tr>
              ) : deductions.map(d => (
                <tr key={d.id} className="table-row-hover">
                  <td className="px-3 py-3 font-mono text-xs text-blue-700 font-medium">{d.invoice_number}</td>
                  <td className="px-3 py-3 text-xs text-gray-700">{d.supplier_name}</td>
                  <td className="px-3 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${SECTION_COLOR[d.section] || 'bg-gray-100 text-gray-800'}`}>
                      {d.section}
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-3 text-xs text-gray-500">{fmtInr(d.cess)}</td>
                  ...
                  <td className="px-3 py-3 text-xs text-gray-500">{d.fiscal_year} Q{d.quarter}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-gray-700">TDS Rate Card — Income Tax Act Sections</h3>
          </div>
          {rates ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(rates).map(([section, info]) => (
                <div key={section} className="rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-3">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full ${SECTION_COLOR[section] || 'bg-gray-100 text-gray-800'}`}>
                      Section {section}
                    </span>
                  </div>
                  <p className="text-sm text-gray-700 font-medium mb-3">{info.description}</p>
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-gray-500">Individual / HUF</span>
                      <span className="font-bold text-gray-900">{info.individual}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Company</span>
                      <span className="font-bold text-gray-900">{info.company}%</span>
                    </div>
                    <div className="flex justify-between border-t pt-2 mt-2">
                      <span className="text-gray-500">+ H&E Cess</span>
