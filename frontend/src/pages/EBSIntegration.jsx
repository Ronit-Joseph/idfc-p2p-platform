...
const EVENT_TYPE_BADGE = {
  ...
  GL_JOURNAL:     'bg-warmgray-100 text-warmgray-700',
...
export default function EBSIntegration() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-warmgray-900">Oracle EBS Integration</h1>
        <p className="text-sm text-warmgray-500 mt-0.5">P2P Platform → On-Prem EBS REST API → Oracle AP/GL/FA</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className={`rounded-xl border p-4 text-center ${s.color === 'red' ? 'bg-red-50 border-red-200' : s.color === 'green' ? 'bg-green-50 border-green-200' : s.color === 'yellow' ? 'bg-yellow-50 border-yellow-200' : 'bg-brand-50 border-brand-200'}`}>
            <div className={`text-2xl font-bold ${s.color === 'red' ? 'text-red-700' : s.color === 'green' ? 'text-green-700' : s.color === 'yellow' ? 'text-yellow-700' : 'text-brand-700'}`}>{s.value}</div>
      ...
      <div className="bg-warmgray-50 border border-warmgray-200 rounded-xl p-4 text-xs text-warmgray-600">
        <div className="font-semibold text-warmgray-700 mb-2 text-sm">Integration Flow</div>
        <div className="flex items-center gap-2 flex-wrap">
          <span className="bg-brand-100 text-brand-800 px-2 py-1 rounded font-medium">P2P Platform Event</span>
          <span>→</span>
          ...
          <span>→</span>
          <span className="bg-warmgray-200 text-warmgray-700 px-2 py-1 rounded font-medium">Oracle AP/GL/FA Open Interface</span>
        </div>
        <p className="mt-2 text-warmgray-500">All postings are idempotent (idempotency key on each event). Failed events auto-retry 3× with exponential backoff, then land in DLQ for manual replay.</p>
      </div>
            ...
            <tbody className="divide-y divide-warmgray-50">
              {data.events.map(e => (
                <tr key={e.id} className={`text-sm ${e.status === 'FAILED' ? 'bg-red-50' : ''}`}>
                  <td className="px-3 py-3 font-mono text-xs text-warmgray-500">{e.id}</td>
                  <td className="px-3 py-3">
                    <span className={`badge text-[11px] ${EVENT_TYPE_BADGE[e.event_type] || 'badge-gray'}`}>
                      {e.event_type.replace(/_/g,' ')}
                    </span>
                  </td>
                  <td className="px-3 py-3 font-mono text-xs text-brand-700">{e.entity_ref}</td>
                  <td className="px-3 py-3 text-xs text-warmgray-600 max-w-[160px]">
                    <div>{e.description}</div>
                    {e.error_message && <div className="text-red-600 text-[11px] mt-0.5">{e.error_message}</div>}
                  </td>
                  <td className="px-3 py-3">
                    <span className="badge badge-gray text-[11px]">{e.ebs_module}</span>
                  </td>
                  <td className="px-3 py-3 font-medium text-xs">{fmtInr(e.amount)}</td>
                  <td className="px-3 py-3">
                    <span className={`flex items-center gap-1 badge text-[11px] ${e.status === 'ACKNOWLEDGED' ? 'badge-green' : e.status === 'FAILED' ? 'badge-red' : 'badge-yellow'}`}>
                      {e.status === 'ACKNOWLEDGED' ? <><CheckCircle className="w-2.5 h-2.5" /> ACK</> :
                       e.status === 'FAILED' ? <><XCircle className="w-2.5 h-2.5" /> FAILED</> :
                       <><Clock className="w-2.5 h-2.5" /> PENDING</>}
                    </span>
                  </td>
                  <td className="px-3 py-3 font-mono text-xs text-warmgray-400">{e.ebs_ref || '—'}</td>
                  <td className="px-3 py-3 text-xs text-warmgray-400">{e.sent_at ? new Date(e.sent_at).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : '—'}</td>
                  <td className="px-3 py-3">
                    {e.status === 'FAILED' && (
                      <button
